<?php include_once("encabezado.php"); ?>
<h2 class="text-center">Registro</h2>
<form action="<?php print RUTA; ?>login/registro/" method="POST">
    <div class="form-group text-left">
      <input type="text" name="nombre" id="nombre" class="form-control" required placeholder="*Escriba su nombre:" 
      value='<?php isset($datos["data"]["nombre"])? print $datos["data"]["nombre"]:""; ?>' />
    </div>

    <div class="form-group text-left">
      <input type="text" name="apellidoPaterno" id="apellidoPaterno" class="form-control" required placeholder="*Apellido Paterno:" 
      value='<?php isset($datos["data"]["apellidoPaterno"])? print $datos["data"]["apellidoPaterno"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="apellidoMaterno" id="apellidoMaterno" class="form-control" placeholder="Apellido Materno:" 
      value='<?php isset($datos["data"]["apellidoMaterno"])? print $datos["data"]["apellidoMaterno"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="email" name="email" id="email" class="form-control" placeholder="*Correo electrónico:"
       value='<?php isset($datos["data"]["email"])? print $datos["data"]["email"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="password" name="clave1" id="clave1" class="form-control" placeholder="*Escriba su contraseña:"
       required value='<?php isset($datos["data"]["clave1"])? print $datos["data"]["clave1"]:""; ?>'/>
    </div>

    <div class="form-group text-left">

      <input type="password" name="clave2" id="clave2" class="form-control" placeholder="Verifique su contraseña:" 
      required value='<?php isset($datos["data"]["clave2"])? print $datos["data"]["clave2"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="direccion" id="direccion" class="form-control" placeholder="*Escriba su dirección:" 
      required value='<?php isset($datos["data"]["direccion"])? print $datos["data"]["direccion"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="ciudad" id="ciudad" class="form-control" placeholder="*Escriba su ciudad:"
       required value='<?php isset($datos["data"]["ciudad"])? print $datos["data"]["ciudad"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="estado" id="estado" class="form-control" placeholder="*Escriba su estado:" 
      required value='<?php isset($datos["data"]["estado"])? print $datos["data"]["estado"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="colonia" id="colonia" class="form-control" placeholder="*Escriba su colonia:" 
      required value='<?php isset($datos["data"]["colonia"])? print $datos["data"]["colonia"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="codpos" id="codpos" class="form-control" placeholder="*Escriba su código postal:" 
      required value='<?php isset($datos["data"]["codpos"])? print $datos["data"]["codpos"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <input type="text" name="pais" id="pais" class="form-control" placeholder="*Escriba su país:" 
      required value='<?php isset($datos["data"]["pais"])? print $datos["data"]["pais"]:""; ?>'/>
    </div>

    <div class="form-group text-left">
      <label for="enviar"></label>
      <input type="submit" value="Enviar datos"  class="btn btn-success" role="button"/>
      <a href="<?php print RUTA; ?>login/" class="btn btn-info">Regresar</a>
    </div>
</form>
<?php include_once("piepagina.php"); ?>